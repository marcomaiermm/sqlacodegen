...
class ImportCollector(OrderedDict):
    def add_import(self, obj):
        ...
        if pkgname.startswith("sqlalchemy.dialects."):
            ...
            pkgname = (
                "sqlalchemy"
                if type_.__name__ in sqlalchemy.__all__
                else type_.__module__
            )
...
class ModelClass(Model):
    ...
    def _tablename_to_classname(cls, tablename, inflect_engine):
        ...
        camel_case_name = "".join(
            part[:1].upper() + part[1:] for part in tablename.split("_")
        )
...
class ManyToOneRelationship(Relationship):
    ...
    def get_common_fk_constraints(table1, table2):
        ...
        c1 = set(
            ...
            if isinstance(c, ForeignKeyConstraint)
            and c.elements[0].column.table == table2
        )
        c2 = set(
            c
            for c in table2.constraints
            if isinstance(c, ForeignKeyConstraint)
            and c.elements[0].column.table == table1
        )
        return c1.union(c2)
...
class ManyToManyRelationship(Relationship):
    def __init__(self, source_cls, target_cls, assocation_table):
        ...
        self.preferred_name = (
            tablename if not colname.endswith("_id") else colname[:-3] + "s"
        )

        # Handle self referential relationships
        if source_cls == target_cls:
            self.preferred_name = (
                "parents" if not colname.endswith("_id") else colname[:-3] + "s"
            )
...
{models}"""
        ...
        for table in metadata.sorted_tables:
            ...
            if noclasses or not table.primary_key or table.name in association_tables:
                ...
                model = self.class_model(
                    table, links[table.name], self.inflect_engine, not nojoined
                )
    ...
    def render_column(self, column, show_name):
        ...
        dedicated_fks = [
            c for c in column.foreign_keys if len(c.constraint.columns) == 1
        ]
        is_unique = any(
        ...
        return "Column({0})".format(
            ", ".join(
                ...
                + (
                    ...
                    if comment and not self.nocomments
                    else []
                )
            )
        )
    ...
    def render_table(self, model):
        ...
        for column in model.table.columns:
            rendered += "{0}{1},\n".format(
                self.indentation, self.render_column(column, True)
            )
        ...
        for constraint in sorted(model.table.constraints, key=_get_constraint_sort_key):
            ...
            if (
                ...
                continue
            rendered += "{0}{1},\n".format(
                self.indentation, self.render_constraint(constraint)
            )

        for index in model.table.indexes:
            if len(index.columns) > 1:
                rendered += "{0}{1},\n".format(
                    self.indentation, self.render_index(index)
                )
